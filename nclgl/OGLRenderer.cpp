...
void OGLRenderer::SetTextureStretch(GLuint target, bool stretch) {
	glBindTexture(GL_TEXTURE_2D, target);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, stretch ? GL_REPEAT : GL_CLAMP);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, stretch ? GL_REPEAT : GL_CLAMP);
	glBindTexture(GL_TEXTURE_2D, 0);
}
...
void DebugDrawData::Draw() {
	...
	glBindBuffer(GL_ARRAY_BUFFER,buffers[Mesh::VERTEX_BUFFER]);
	glBufferData(GL_ARRAY_BUFFER,lines.size()*sizeof(Vector3), &lines[0], GL_DYNAMIC_DRAW);
	glVertexAttribPointer(Mesh::VERTEX_BUFFER, 3, GL_FLOAT, GL_FALSE, 0, 0); 
	glEnableVertexAttribArray(Mesh::VERTEX_BUFFER);

	glBindBuffer(GL_ARRAY_BUFFER,buffers[Mesh::COLOUR_BUFFER]);
	glBufferData(GL_ARRAY_BUFFER,colours.size()*sizeof(Vector3), &colours[0], GL_DYNAMIC_DRAW);
	glVertexAttribPointer(Mesh::COLOUR_BUFFER, 3, GL_FLOAT, GL_FALSE, 0, 0); 
	glEnableVertexAttribArray(Mesh::COLOUR_BUFFER);
...
void OGLRenderer::SetShaderLight(const Light& l) {
	...
	glUniform4fv(glGetUniformLocation(currentShader->GetProgram(), "diffuseColour"), 1, (float*) &l.GetDiff());
	glUniform4fv(glGetUniformLocation(currentShader->GetProgram(), "specColour"), 1, ( float*) & l.GetSpec());
